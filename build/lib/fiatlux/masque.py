...
class Mask(object):
    ...
    def __init__(self, field_size, **kwargs):
        ...
        self._complex_transparency = np.zeros((self._field_size,
                                               self._field_size))
...
class Fourier(Mask):
    ...
    def __init__(self, field_size, **kwargs):
        ...
        self._modes = np.zeros((field_size, field_size,
                                (self._n_modes + 1)**2))
        self._coefficients = kwargs.get('coefficients',
                                        np.ones(self._n_modes))
    ...
    def _build_modes(self):
        ...
        for k in range(0, self._n_modes):
            for l in range(0, self._n_modes):
                ...
                tmp[N//2+4*k, N//2+4*l] = 1/2
                self._modes[:, :, idx] = np.real(np.fft.fft2(
                                                 np.fft.fftshift(tmp)))
                idx += 1
    ...
    coefficients = property(_get_coefficients, _set_coefficients,
                            _del_coefficients,
...
class Pyramid(Mask):
    ...
    def _build_pyramid(self):
        ...
        angle_grid = np.arctan2(x_grid*np.sin(-pyramid_orientation) +
    ...
    angle = property(_get_angle, _set_angle, _del_angle, "shape of the mask. "
                     "None if no defined shape")
    faces = property(_get_faces, _set_faces, _del_faces, "shape of the mask. "
                     "None if no defined shape")
    modulation = property(_get_modulation, _set_modulation, _del_modulation,
                          "shape of the mask. None if no defined shape")
...
class Tilt(Mask):
    ...
    angles = property(_get_angles, _set_angles, _del_angles, "shape of the "
                      "mask. None if no defined shape")
...
class TipTiltMirror(Mask):
    ...
    def __init__(self, field_size, **kwargs):
        ...
        elif self._modulation_shape is 'square':
...
#        tilt = tilt
    ...
    angles = property(_get_angles, _set_angles, _del_angles, "shape of the "
                      "mask. None if no defined shape")
    modulation_shape = property(_get_modulation_shape, _set_modulation_shape,
                                _del_modulation_shape,
                                "shape of the mask. None if no defined shape")
    modulation_size = property(_get_modulation_size, _set_modulation_size,
...
class Zelda(Mask):
    ...
    def _build_zelda(self):
        ...
        """
        N = self._field_size
        x = np.linspace(-N//2+.5, N//2-.5, N)
        y = np.linspace(-N//2+.5, N//2-.5, N)
        x_grid, y_grid = np.meshgrid(x, y)
    ...
    patch_shift = property(_get_patch_shift, _set_patch_shift,
                            _del_patch_shift,
                            "shape of the mask. None if no defined shape")
...
class Zernike(Mask):
    ...
    def _build_mean_matrix(self):
        ...
        for z in range(n0,N-n0):
            self._mean_matrix[z-n0,
                        -n0+(z+1)*(z+2)//2-1-z-offset:
                            (z+1)*(z+2)//2-n0-offset]=1/z
    ...
    mean_matrix = property(_get_mean_matrix, _set_mean_matrix,
                            _del_mean_matrix,
                            "shape of the mask. None if no defined shape")
