...
class Propagator(object):
    ...
    def __init__(self, LCT_type, field, **kwargs):
        ...
        # FOURIER TRANSFORMATION
            ...
            # LCT
            self._LCT = [[0, self._wavelength*self._f],
        ...
        # FRACTIONNAL FOURIER TRANSFORMATION
            ...
            # LCT
            self._LCT = [[np.cos(self._p*np.pi/2),
    ...
    def propagate(self, field):
        if self._LCT_type is 'FFT':
            FFTCube = np.zeros((field.field_size, field.field_size,
                                np.shape(field.complex_amplitude)[2]),
                               dtype=complex)
            for k in range(0, np.shape(field.complex_amplitude)[2]):
                FFTCube[:, :, k] = np.fft.fftshift(np.fft.fft2(np.fft.fftshift(
                    field.complex_amplitude[:, :, k])))
            return FFTCube
        elif self._LCT_type is 'IFFT':
            IFFTCube = np.zeros((field.field_size, field.field_size,
                                np.shape(field.complex_amplitude)[2]),
                                dtype=complex)
            for k in range(0, np.shape(field.complex_amplitude)[2]):
                IFFTCube[:, :, k] = np.fft.fftshift(np.fft.fft2(
                        np.fft.fftshift(field.complex_amplitude[:, :, k])))
            return IFFTCube
        ...
        # FST
            ...
            FSTCube = np.zeros((field.field_size, field.field_size,
                                np.shape(field.complex_amplitude)[2]),
                               dtype=complex)
            # NUMBER OF PIXELS
            N = field.field_size
            ...
            if z < Lx**2/(wavelength*N):
                for k in range(0, np.shape(field.complex_amplitude)[2]):
                    ...
                    FSTCube[:, :, k] = np.fft.fftshift(np.fft.ifft2(
                            np.fft.fftshift(np.fft.fftshift(
                ...
                for k in range(0, np.shape(field.complex_amplitude)[2]):
                    ...
                    FSTCube[:, :, k] = np.fft.fftshift(np.fft.fft2(
                            np.fft.fftshift(
                                    field.complex_amplitude[:, :, k] *
                                    np.exp(1j*(np.pi/(wavelength*z)) *
                                           (Lx/N)**2 * x_grid**2) *
    ...
    LCT_type = property(_get_LCT_type, _set_LCT_type, _del_LCT_type,
