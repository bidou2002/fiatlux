...
def R(n, m, rho):
    ...
    m = int(torch.abs(torch.as_tensor(m)))
    n = int(torch.abs(torch.as_tensor(n)))
    output = torch.zeros(rho.shape)
    if (n - m) % 2 != 0:
        return 0
    else:
...
def zernike(
    n, m, npix=100, rho=None, theta=None, outside=0, noll_normalize=True, **kwargs
):
    ...
    outside : float
        ...
        Default is `torch.nan`, but you may also find it useful for this to
        be 0.0 sometimes.
    noll_normalize : bool
    ...
    if theta is None and rho is None:
        x = (torch.arange(npix, dtype=torch.float64) - (npix - 1) / 2.0) / (
            (npix - 1) / 2.0
        )
        y = x
    ...
    if m == 0:
        if n == 0:
            ...
            norm_coeff = torch.sqrt(torch.tensor(n + 1)) if noll_normalize else 1
            zernike_result = norm_coeff * R(n, m, rho) * aperture
    elif m > 0:
        norm_coeff = (
            torch.sqrt(torch.tensor(2)) * torch.sqrt(torch.tensor(n + 1))
            if noll_normalize
            else 1
        )
        zernike_result = (
            norm_coeff
            * R(n, m, rho)
            * torch.cos(torch.abs(torch.as_tensor(m)) * theta)
            * aperture
        )
    else:
        norm_coeff = (
        ...
        zernike_result = (
            norm_coeff
            * R(n, m, rho)
            * torch.sin(torch.abs(torch.as_tensor(m)) * theta)
            * aperture
        )
...
def noll_indices(j):
    ...
    n = int(torch.ceil((-1 + torch.sqrt(torch.tensor(1 + 8 * j))) / 2) - 1)
    if n == 0:
        m = 0
    else:
        ...
        for i in range(int(torch.floor(torch.tensor(n / 2.0)))):
...
def zernike_basis(nterms=15, npix=512, rho=None, theta=None, **kwargs):
    ...
    zern_output = torch.zeros((nterms,) + shape)
