...
class Field:
    ...
    channel and latent sample. Consequently, the photon rate in channel ``k``
    for each latent sample is
    ``intensity()[..., k, :, :].sum((-2, -1)) * grid.dx * grid.dy``.
    Wavelength is axis ``-3``; spatial axes are ``(-2, -1)``.
